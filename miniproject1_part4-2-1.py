...
def cosine_similarity(x, y):
    ...
    cos_sim = np.dot(x, y) / (la.norm(x) * la.norm(y))

    # Exponentiate the cosine similarity
    exp_cos_sim = np.exp(cos_sim)
...
def download_glove_embeddings_gdrive(model_type):
    ...
    embeddings_temp = "C:\EE596\HW1\embeddings_50d_temp.npy"
    word_index_temp = "C:\EE596\HW1\word_index_dict_50d_temp.pkl"
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type=50):
    ...
    # Count the number of words that have embeddings
    valid_words_count = 0

    # Get embeddings for each word and add them
    for word in words:
        if word.lower() in word_index_dict:
            embedding += embeddings[word_index_dict[word.lower()]]
            valid_words_count += 1

    # Divide by number of valid words to get average
